...
\\
\codebox{
  docker exec -it cockroach\_roach1\_1 ./cockroach workload fixtures import tpcc --warehouses=10 'postgresql://root@roach1:26257?sslmode=disable'
}
\begin{figure}[H]
  \centering
...
\codebox{
  perf record -o ./benchmark/cockroach-benchmark-tpcc-bare-metal-\$(uname -r).perf -e tlb:tlb\_flush,dTLB-loads,dTLB-load-misses,iTLB-load-misses,cache-misses,page-faults docker exec -it cockroach\_roach1\_1 cockroach workload run tpcc --warehouses=10 --ramp=3m --duration=10m 'postgresql://root@roach1:26257?sslmode=disable'
}