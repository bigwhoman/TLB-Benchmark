...
sudo strace -f -p "$pids" 2> ./benchmark/cassandra-benchmark-5-5-vm-$(uname -r).strace &
strace_id=$!
sudo perf record -p $pids -o ./benchmark/cassandra-benchmark-5-5-vm-$(uname -r).perf -e tlb:tlb_flush,dTLB-loads,dTLB-load-misses,iTLB-load-misses,cache-misses,page-faults &
perf_id=$!
echo $perf_id
sleep $duration
# Check if strace process is still running before trying to kill it
